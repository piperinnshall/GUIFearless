...
interface Lerp extends UnaryOperator<Float> {
  ...
  Lerp EASE_IN_OUT_QUAD = t -> t < 0.5f ? 2 * t * t : -1 + (4 - 2 * t) * t;
  Lerp EASE_IN_CUBIC = t -> t * t * t;
  Lerp EASE_OUT_CUBIC = t -> 1 - (1 - t) * (1 - t) * (1 - t);
  Lerp EASE_IN_OUT_CUBIC = t -> t < 0.5f ? 4 * t * t * t : 1 - (float) Math.pow(-2 * t + 2, 3) / 2;
  Lerp EASE_IN_QUART = t -> t * t * t * t;
  Lerp EASE_OUT_QUART = t -> 1 - (1 - t) * (1 - t) * (1 - t) * (1 - t);
  Lerp EASE_IN_OUT_QUART = t -> t < 0.5f ? 8 * t * t * t * t : 1 - (float) Math.pow(-2 * t + 2, 4) / 2;
